      ...
      <div class="row pt-2">
        <div class="col-lg-6 col-md-6 align-self-center">
          <p class="mb-0 py-3 text-center text-md-left">
            <?php
              if (!get_theme_mod('charitable_non_profit_footer_text') ) { ?>    
                <a href="<?php echo esc_url(__('https://www.wpelemento.com/elementor/free-charitable-wordpress-theme/', 'charitable-non-profit' )); ?>" target="_blank">
                  <?php esc_html_e('Charitable Non Profit WordPress Theme','charitable-non-profit'); ?>
                </a>
           <?php } else {
                echo esc_html(get_theme_mod('charitable_non_profit_footer_text'));
              }
            ?>
            <?php if ( get_theme_mod('charitable_non_profit_copyright_enable', true) == true ) : ?>
            <?php
              /* translators: %s: WP Elemento */
              printf( esc_html__( ' By %s', 'charitable-non-profit' ), 'WP Elemento' ); ?>
            <?php endif; ?>
          </p>
        </div>
        <div class="col-lg-6 col-md-6 align-self-center text-center text-md-right">
          <?php if ( get_theme_mod('charitable_non_profit_copyright_enable', true) == true ) : ?>
            <a href="<?php echo esc_url(__('https://wordpress.org','charitable-non-profit') ); ?>" rel="generator"><?php  /* translators: %s: WordPress */ printf( esc_html__( 'Proudly powered by %s', 'charitable-non-profit' ), 'WordPress' ); ?></a>
          <?php endif; ?>
        </div>
      </div>
      <?php if ( get_theme_mod('charitable_non_profit_scroll_enable_setting')) : ?>
        <div class="scroll-up">
